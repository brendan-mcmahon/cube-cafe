...
function Actions() {
  ...
  const dispatchAction = (action) => {
    if (state.playPhase === PlayPhases.SELECT_ACTION && state.availableActions.includes(action)) {
  ...
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="210" height="210">
      ...
      {/* meeple */}
      <path
        onClick={() => dispatchAction(PlayerActions.SEAT_CUSTOMER)}
        filter={state.availableActions.includes(PlayerActions.SEAT_CUSTOMER) ? "url(#glow)" : ""}
        fillRule="evenodd"
        fill={state.availableActions.includes(PlayerActions.SEAT_CUSTOMER) ? "#0C0" : "#999"}
      ...
      <g>
        <path
          onClick={() => dispatchAction(PlayerActions.TAKE_ORDER)}
          fill="transparent"
          d="M-20 -20 H 20 V 20 H -20"
          transform="translate(105 105) rotate(72 55.055 40)"
        />
        <path
          onClick={() => dispatchAction(PlayerActions.TAKE_ORDER)}
          filter={state.availableActions.includes(PlayerActions.TAKE_ORDER) ? "url(#glow)" : ""}
          fill={state.availableActions.includes(PlayerActions.TAKE_ORDER) ? "#0C0" : "#999"}
          fillRule="evenodd"
      ...
      <g>
        <rect
          ...
          fill="none"
          pointerEvents="all"
          onClick={() => dispatchAction(PlayerActions.COOK)}
          transform="translate(105 105) rotate(144 12.997 40)"
        />
        <path
          onClick={() => dispatchAction(PlayerActions.COOK)}
          filter={state.availableActions.includes(PlayerActions.COOK) ? "url(#glow)" : ""}
          fillRule="evenodd"
          fill={state.availableActions.includes(PlayerActions.COOK) ? "#0C0" : "#999"}
      ...
      <g>
        <rect
          ...
          fill="none"
          pointerEvents="all"
          onClick={() => dispatchAction(PlayerActions.SERVE)}
          transform="translate(105 105) rotate(-144 -12.997 40)"
        />
        <path
          filter={state.availableActions.includes(PlayerActions.SERVE) ? "url(#glow)" : ""}
          ...
          transform="translate(105 105) rotate(-144 -12.997 40)"
          fill={state.availableActions.includes(PlayerActions.SERVE) ? "#0C0" : "#999"}
        ></path>
        <text
          fill={state.availableActions.includes(PlayerActions.SERVE) ? "#0C0" : "#999"}
          textAnchor="middle"
      ...
      <g>
        <path
          onClick={() => dispatchAction(PlayerActions.REFILL)}
          filter={state.availableActions.includes(PlayerActions.REFILL) ? "url(#glow)" : ""}
          ...
          transform="translate(105 105) rotate(-72 -55.055 40)"
          fill={state.availableActions.includes(PlayerActions.REFILL) ? "#0C0" : "#999"}
        ></path>
        <text
          fill="#fff"
          textAnchor="middle"
